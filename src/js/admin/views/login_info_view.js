...
const LoginInfoView = (() => {
  ...
  function render() {
    ...
    return `
      ${AdminUtils.renderTopBar('Login Information', 'ph-sign-in', 'User login activity and session logs')}
            ...
            <tbody>
              ${rows.map(row => `
                <tr class="admin-table-row">
                  ...
                  <td class="admin-table-cell font-15px fw-semibold ">${AdminUtils.escapeHtml(row.lastLogin)}</td>
                  <td class="admin-table-cell font-15px fw-semibold ">${AdminUtils.escapeHtml(row.machineAddress)}</td>
